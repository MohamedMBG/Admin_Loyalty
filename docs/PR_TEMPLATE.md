...
Example:
This PR adds pagination to the customer list screen and fixes a listener leak in the dashboard fragment.
...
Example:
The customer list fetched the entire collection on open, which was slow for large shops and increased Firestore read costs.
...
2. Include the account/role needed (admin or cashier) and any required test data.
...
State any performance impact (query limits, threading, listener lifecycle) and any security-relevant change (roles, Firestore rules, input validation). Write "None" if not applicable.
